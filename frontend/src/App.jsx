...
export default function App() {
  ...
  const [encodeStatus, setEncodeStatus] = useState('idle');
  ...
  const saveOperation = (type, status, details) => {
    ...
    const newHistory = [newOp, ...operationsHistory].slice(0, 50);
  ...
  const pushLog = (setter, line) => setter(prev => [...prev.slice(-49), line]);
  ...
  const navigateToTab = (tab) => {
    setActiveTab(tab);
    navigate('/StegoSphere');
  };

  return (
    <Routes>
      {/* Main Route - StegoSphere Operations */}
      <Route path="/StegoSphere" element={
            ...
            <div className="flex justify-between items-start mb-12 border-b border-green-900/50 pb-6">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <h1
                    className="glitch text-4xl sm:text-5xl font-black uppercase leading-none cursor-pointer"
            ...
            <div className="flex flex-col xl:flex-row gap-6">
              ...
              <div className="w-full xl:w-64 shrink-0 flex flex-col gap-6">
                ...
                <Card title="SYS_SPECS" icon={Activity} className="hidden xl:flex text-[10px] text-green-700 space-y-3">
                  <div className="flex justify-between border-b border-green-900/30 pb-1"><span>PROTOCOL:</span> <span className="text-green-500">LSB_V2</span></div>
                  <div className="flex justify-between border-b border-green-900/30 pb-1"><span>CIPHER:</span> <span className="text-green-500">AES-256-GCM</span></div>
                  <div className="flex justify-between border-b border-green-900/30 pb-1"><span>KDF_ITERATIONS:</span> <span className="text-cyan-500">100,000</span></div>
                  <div className="flex justify-between border-b border-green-900/30 pb-1"><span>HASH:</span> <span className="text-green-500">SHA-256</span></div>
                  <div className="mt-4 opacity-50 bg-green-900/20 p-2 text-center text-green-500 border border-green-900/30">STRICT AUTHENTICATION</div>
                </Card>
              </div>
                          ...
                          <tbody className="text-green-400/80">
                            {operationsHistory.map(op => (
                              <tr key={op.id} className="border-b border-green-900/20 hover:bg-green-900/10 transition-colors">
                                ...
                                <td className="p-3">
                                  <span className={`px-2 py-0.5 border ${op.type === 'ENCODE' ? 'border-green-700 text-green-500' : 'border-cyan-700 text-cyan-500'}`}>{op.type}</span>
                                </td>
                                <td className="p-3">
                                  {op.status === 'SUCCESS' ? <span className="text-green-400 flex items-center gap-1"><CheckCircle2 size={12} /> OK</span> : <span className="text-red-500 flex items-center gap-1"><XCircle size={12} /> FAIL</span>}
                                </td>
                    ...
                    {activeTab === 'encode' && (
                      <div className="animate-fade-in flex flex-col gap-6">
                        <div className="bg-black/60 border border-green-900/50 p-3 flex justify-between items-center text-xs font-mono">
                          <div className="flex items-center gap-3"><span className="animate-pulse w-2 h-2 bg-green-500 block rounded-full"></span><span className="text-green-500 tracking-wider">MODULE_ENC: ACTIVE</span></div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                          <div className="lg:col-span-5 flex flex-col gap-6">
                            <Card title="CARRIER_INPUT" icon={Upload}>
                              <div className={`upload-zone border-2 border-dashed ${encodeDrag ? 'drag-over border-green-400 bg-green-900/20' : 'border-green-900/60 bg-black/40'} flex flex-col items-center justify-center overflow-hidden transition-all duration-300 group`} style={{ height: '320px', position: 'relative' }} onDragOver={handleDragEvents(setEncodeDrag).onDragOver} onDragEnter={handleDragEvents(setEncodeDrag).onDragEnter} onDragLeave={handleDragEvents(setEncodeDrag).onDragLeave} onDrop={(e) => { e.preventDefault(); e.stopPropagation(); setEncodeDrag(false); handleEncodeFile(e.dataTransfer.files[0]); }} onClick={() => encodeFileRef.current.click()}>
                                {encodePreview ? (
                                  <div className="relative w-full h-full group-hover:scale-[1.02] transition-transform duration-500">
                                    ...
                                    <div className="absolute top-2 right-2 bg-black/80 border border-green-500/50 px-2 py-1 text-[10px] text-green-400">LOCKED</div>
                                  </div>
                                ) : (
                                  <div className="text-center p-6 select-none flex flex-col items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-green-800 text-[10px] uppercase font-mono tracking-widest leading-relaxed">Click or drop file<br />Valid formats: PNG, JPG, WEBP<br />Max size: 15.0 MB</p>
                                    </div>
                                  </div>
                                )}
                              ...
                              {encodeImage && (
                                <div className="mt-4 bg-black/60 border border-green-900/40 p-3 flex justify-between items-center text-[10px] text-green-600 font-mono">
                                  <span className="truncate max-w-[150px] font-bold text-green-500" title={encodeImage.name}>{encodeImage.name}</span>
                                  <span className="flex items-center gap-2">
                                    <span className="opacity-70">{(encodeImage.size / (1024 * 1024)).toFixed(2)} MB</span>
                                    <button className="text-red-500/70 hover:text-red-400 p-1" onClick={(e) => { e.stopPropagation(); handleClearEncode(); }} title="Clear"><XCircle size={14} /></button>
                                  </span>
                                </div>
                              )}
                            </Card>
                              ...
                              <div className="flex flex-col gap-6 flex-1 h-full">
                                <div className="flex-1 flex flex-col min-h-[200px]">
                                  ...
                                  <div className="relative flex-1 group shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] border border-green-900/60 hover:border-green-500/50 transition-colors">
                                    <Textarea className="h-full bg-transparent border-none w-full !bg-black/40 focus:bg-black/60" placeholder="> BEGIN ENTRY SEQUENCE...&#10;> CLASSIFIED DATA FOR INJECTION" value={message} onChange={e => setMessage(e.target.value)} />
                                ...
                                <div>
                                  ...
                                  <Input type="password" placeholder="ENTER SECURE KEY FOR PBKDF2 DERIVATION..." value={encodePassword} onChange={e => setEncodePassword(e.target.value)} />
                                </div>

                                <div className="pt-4 border-t border-green-900/30">
                                  {!encodeResultUrl ? (
                                    <Button variant="primary" icon={Zap} onClick={handleEncode} isLoading={encodeStatus === 'processing'} disabled={!encodeImage || !message || encodeStatus === 'processing'}>
                                    ...
                                    <div className="animate-fade-in border border-green-500/40 bg-green-500/5 relative overflow-hidden">
                                      ...
                                      <div className="p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6">
                                        ...
                                        <div className="flex-1 text-center sm:text-left">
                                          ...
                                          <p className="text-green-600/80 text-xs font-mono mb-4">Payload embedded and verified within carrier image. Extraction requires designated key.</p>
                                          <div className="flex flex-col sm:flex-row gap-3">
                                            <a href={encodeResultUrl} download={`secured_output_${Date.now()}.png`} className="stego-btn !py-2.5 !text-[10px] text-center w-full">
                                              <span className="flex justify-center items-center gap-2"><Download size={14} /> DOWNLOAD ARTIFACT</span>
                                            </a>
                                            <button className="border border-green-900 text-green-700 hover:text-green-400 hover:border-green-600 px-4 py-2 text-[10px] font-orbitron tracking-widest bg-black/50 transition-colors w-full sm:w-auto shrink-0" onClick={handleClearEncode}>NEW OP</button>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                    ...
                    {activeTab === 'decode' && (
                      <div className="animate-fade-in flex flex-col gap-6">
                        <div className="bg-black/60 border border-cyan-900/50 p-3 flex justify-between items-center text-xs font-mono">
                          <div className="flex items-center gap-3"><span className="animate-pulse w-2 h-2 bg-cyan-500 block rounded-full"></span><span className="text-cyan-500 tracking-wider">MODULE_DEC: ACTIVE</span></div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                          <div className="lg:col-span-5 flex flex-col gap-6">
                            <Card title="ARTIFACT_ANALYSIS" icon={Search} glowColor="var(--cyan-dim)" className="!border-cyan-900/40">
                              <div className={`upload-zone border-2 border-dashed ${decodeDrag ? 'drag-over border-cyan-500 bg-cyan-900/10' : 'border-cyan-900/40 bg-black/40'} flex flex-col items-center justify-center overflow-hidden transition-all duration-300 group`} style={{ height: '320px', position: 'relative' }} onDragOver={handleDragEvents(setDecodeDrag).onDragOver} onDragEnter={handleDragEvents(setDecodeDrag).onDragEnter} onDragLeave={handleDragEvents(setDecodeDrag).onDragLeave} onDrop={(e) => { e.preventDefault(); e.stopPropagation(); setDecodeDrag(false); handleDecodeFile(e.dataTransfer.files[0]); }} onClick={() => decodeFileRef.current.click()}>
                                {decodePreview ? (
                                  <div className="relative w-full h-full group-hover:scale-[1.02] transition-transform duration-500">
                                    ...
                                    <div className="absolute bottom-2 left-2 bg-black/80 border border-cyan-500/50 px-2 py-1 text-[10px] text-cyan-400 flex items-center gap-1"><Activity size={10} className="animate-pulse" /> SCANNING VISUAL LAYER</div>
                                  </div>
                                ) : (
                                  <div className="text-center p-6 select-none flex flex-col items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-cyan-800 text-[10px] uppercase font-mono tracking-widest leading-relaxed">Click or drop file<br />Valid formats: PNG, BMP<br />Lossless required</p>
                                    </div>
                                  </div>
                                )}
                              ...
                              {decodeImage && (
                                <div className="mt-4 bg-black/60 border border-cyan-900/40 p-3 flex justify-between items-center text-[10px] text-cyan-600 font-mono">
                                  <span className="truncate max-w-[150px] font-bold text-cyan-500" title={decodeImage.name}>{decodeImage.name}</span>
                                  <span className="flex items-center gap-2">
                                    <span className="opacity-70">{(decodeImage.size / (1024 * 1024)).toFixed(2)} MB</span>
                                    <button className="text-red-500/70 hover:text-red-400 p-1" onClick={(e) => { e.stopPropagation(); handleClearDecode(); }} title="Clear"><XCircle size={14} /></button>
                                  </span>
                                </div>
                              )}
                            </Card>
                            ...
                            <Card title="EXTRACTION_SEQ_INIT" icon={Eye} glowColor="var(--cyan-dim)" className="flex-1 !border-cyan-900/40" showGlow={false}>
                              <div className="flex flex-col gap-6 h-full">
                                <div className="bg-cyan-900/10 border border-cyan-900/30 p-5 relative overflow-hidden">
                                  <div className="absolute -right-4 -top-4 opacity-5 text-cyan-500"><Unlock size={80} /></div>
                                  <Label icon={Lock} required>Authentication Key (AEAD Decoder)</Label>
                                  <Input type="password" placeholder="ENTER PASSPHRASE TO BREAK CIPHER..." value={decodePassword} onChange={e => setDecodePassword(e.target.value)} className="!border-cyan-900/60 focus:!border-cyan-500 focus:!shadow-[0_0_12px_rgba(0,229,255,0.2)] !text-cyan-400 cursor-pointer" style={{ width: '100%' }} />
                                </div>
                                <div>
                                  <Button variant="secondary" icon={Activity} onClick={handleDecode} isLoading={decodeStatus === 'processing'} disabled={!decodeImage || decodeStatus === 'processing'}>
                                  ...
                                  <div className={`flex-1 min-h-[220px] bg-black/60 border transition-all duration-300 relative ${decodeError ? 'border-red-900/50 shadow-[inset_0_0_30px_rgba(255,0,60,0.05)]' : decodeResult ? 'border-cyan-500/40 shadow-[inset_0_0_30px_rgba(0,229,255,0.05)] bg-[#051015]' : 'border-cyan-900/30 shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]'} flex flex-col`}>
                                    {decodeError && (
                                      <div className="absolute inset-0 p-6 flex flex-col justify-center items-center text-center animate-fade-in bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0IiBoZWlnaHQ9IjQiPgo8cmVjdCB3aWR0aD0iNCIgaGVpZ2h0PSI0IiBmaWxsPSJ0cmFuc3BhcmVudCIvPgo8cmVjdCB3aWR0aD0iMSIgaGVpZ2h0PSIxIiBmaWxsPSJyZ2JhKDI1NSwgMCwgNjAsIDAuMSkiLz4KPC9zdmc+')]">
                                        ...
                                        <p className="text-red-500/70 font-mono text-[10px] max-w-sm leading-relaxed border border-red-900/50 bg-black/80 px-4 py-3">
                                          {decodeError.toLowerCase().includes('incorrect password') ? 'CRYPTOGRAPHIC FAILURE: Key derivation mismatch. The provided authentication token is invalid or artifact architecture has been modified.' : decodeError}
                                        </p>
                                      </div>
                                    )}
                                    ...
                                    {decodeResult && (
                                      <div className="flex-1 flex flex-col relative h-full animate-fade-in">
                                        ...
                                        <div className="p-5 flex-1 overflow-auto custom-scrollbar-cyan text-cyan-300 font-mono text-sm leading-relaxed whitespace-pre-wrap select-text">{decodeResult}</div>
                                        <div className="p-3 border-t border-cyan-900/30 bg-black/40 flex justify-end">
                                          <button onClick={() => navigator.clipboard.writeText(decodeResult)} className="text-[10px] bg-cyan-900/20 text-cyan-400 border border-cyan-700 hover:bg-cyan-900 hover:text-cyan-300 px-4 py-2 transition-colors font-orbitron tracking-widest">COPY TO CLIPBOARD</button>
                                        </div>
                                      </div>
                                    )}
                    ...
                    {activeTab === 'analyze' && (
                      <div className="animate-fade-in flex flex-col gap-6">
                        <div className="bg-black/60 border border-yellow-900/50 p-3 flex justify-between items-center text-xs font-mono">
                          <div className="flex items-center gap-3"><span className="animate-pulse w-2 h-2 bg-yellow-500 block rounded-full"></span><span className="text-yellow-500 tracking-wider">MODULE_ANL: ACTIVE</span></div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                          <div className="lg:col-span-5 flex flex-col gap-6">
                            <Card title="FORENSIC_TARGET" icon={Upload} glowColor="var(--yellow-dim)" className="!border-yellow-900/40">
                              <div className={`upload-zone border-2 border-dashed ${analyzeDrag ? 'drag-over border-yellow-500 bg-yellow-900/10' : 'border-yellow-900/40 bg-black/40'} flex flex-col items-center justify-center overflow-hidden transition-all duration-300 group`} style={{ height: '320px', position: 'relative' }} onDragOver={handleDragEvents(setAnalyzeDrag).onDragOver} onDragEnter={handleDragEvents(setAnalyzeDrag).onDragEnter} onDragLeave={handleDragEvents(setAnalyzeDrag).onDragLeave} onDrop={(e) => { e.preventDefault(); e.stopPropagation(); setAnalyzeDrag(false); handleAnalyzeFile(e.dataTransfer.files[0]); }} onClick={() => analyzeFileRef.current.click()}>
                                {analyzePreview ? (
                                  <div className="relative w-full h-full group-hover:scale-[1.02] transition-transform duration-500">
                                  ...
                                  <div className="text-center p-6 select-none flex flex-col items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-yellow-800 text-[10px] uppercase font-mono tracking-widest leading-relaxed">Click or drop file<br /></p>
                                    </div>
                                  </div>
                                )}
                              ...
                              {analyzeImage && (
                                <div className="mt-4 bg-black/60 border border-yellow-900/40 p-3 flex justify-between items-center text-[10px] text-yellow-600 font-mono">
                                  ...
                                  <span className="flex items-center gap-2">
                                    ...
                                    <button className="text-red-500/70 hover:text-red-400 p-1" onClick={(e) => { e.stopPropagation(); handleClearAnalyze(); }} title="Clear"><XCircle size={14} /></button>
                                  </span>
                                </div>
                              )}
                            </Card>
                          ...
                          <div className="lg:col-span-7 flex flex-col gap-6">
                            <Card title="DATA_FORENSICS" icon={Activity} glowColor="var(--yellow-dim)" className="flex-1 !border-yellow-900/40" showGlow={false}>
                              <div className="flex flex-col gap-6 h-full">
                                <div>
                                  <Button variant="yellow" icon={BarChart2} onClick={handleAnalyze} isLoading={analyzeStatus === 'processing'} disabled={!analyzeImage || analyzeStatus === 'processing'}>
                    ...
                    {activeTab === 'sanitize' && (
                      <div className="animate-fade-in flex flex-col gap-6">
                        <div className="bg-black/60 border border-red-900/50 p-3 flex justify-between items-center text-xs font-mono">
                          <div className="flex items-center gap-3"><span className="animate-pulse w-2 h-2 bg-red-500 block rounded-full"></span><span className="text-red-500 tracking-wider">MODULE_SNT: ACTIVE</span></div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                          <div className="lg:col-span-5 flex flex-col gap-6">
                            <Card title="DESTRUCTION_TARGET" icon={Upload} glowColor="var(--red-dim)" className="!border-red-900/40">
                              <div className={`upload-zone border-2 border-dashed ${sanitizeDrag ? 'drag-over border-red-500 bg-red-900/10' : 'border-red-900/40 bg-black/40'} flex flex-col items-center justify-center overflow-hidden transition-all duration-300 group`} style={{ height: '320px', position: 'relative' }} onDragOver={handleDragEvents(setSanitizeDrag).onDragOver} onDragEnter={handleDragEvents(setSanitizeDrag).onDragEnter} onDragLeave={handleDragEvents(setSanitizeDrag).onDragLeave} onDrop={(e) => { e.preventDefault(); e.stopPropagation(); setSanitizeDrag(false); handleSanitizeFile(e.dataTransfer.files[0]); }} onClick={() => sanitizeFileRef.current.click()}>
                                {sanitizePreview ? (
                                  <div className="relative w-full h-full group-hover:scale-[1.02] transition-transform duration-500">
                                    ...
                                    <div className="absolute top-2 right-2 bg-black/80 border border-red-500/50 px-2 py-1 text-[10px] text-red-500 font-bold blink">TARGET LOCKED</div>
                                  </div>
                                ) : (
                                  <div className="text-center p-6 select-none flex flex-col items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-red-800 text-[10px] uppercase font-mono tracking-widest leading-relaxed">Click or drop file<br /></p>
                                    </div>
                                  </div>
                                )}
                              ...
                              {sanitizeImage && (
                                <div className="mt-4 bg-black/60 border border-red-900/40 p-3 flex justify-between items-center text-[10px] text-red-600 font-mono">
                                  ...
                                  <span className="flex items-center gap-2">
                                    ...
                                    <button className="text-red-500/70 hover:text-red-400 p-1" onClick={(e) => { e.stopPropagation(); handleClearSanitize(); }} title="Clear"><XCircle size={14} /></button>
                                  </span>
                                </div>
                              )}
                            </Card>
                          ...
                          <div className="lg:col-span-7 flex flex-col gap-6">
                            <Card title="LSB_SCRUB_SEQUENCE" icon={Activity} glowColor="var(--red-dim)" className="flex-1 !border-red-900/40" showGlow={false}>
                              <div className="flex flex-col gap-6 h-full">
                                <div>
                                  <p className="text-red-500/80 font-mono text-xs mb-4">This action will zero out Least Significant Bits of all color channels, effectively destroying any hidden signals while preserving the visual composition of the host file.</p>
                                  <Button variant="red" icon={ShieldOff} onClick={handleSanitize} isLoading={sanitizeStatus === 'processing'} disabled={!sanitizeImage || sanitizeStatus === 'processing'}>
                                    ...
                                    <div className="animate-fade-in border border-red-500/40 bg-red-500/5 relative overflow-hidden">
                                      ...
                                      <div className="p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6">
                                        ...
                                        <div className="flex-1 text-center sm:text-left">
                                          ...
                                          <div className="flex flex-col sm:flex-row gap-3">
                                            <a href={sanitizeResultUrl} download={`clean_${sanitizeImage?.name?.split('.')[0] || 'img'}.png`} className="stego-btn !border-red-500 !text-red-500 hover:!bg-red-500 hover:!text-black !py-2.5 !text-[10px] text-center w-full">
                                              <span className="flex justify-center items-center gap-2"><Download size={14} /> SECURE DOWNLOAD</span>
                                            </a>
                                            <button className="border border-red-900 text-red-500 hover:text-red-400 hover:border-red-600 px-4 py-2 text-[10px] font-orbitron tracking-widest bg-black/50 transition-colors w-full sm:w-auto shrink-0" onClick={handleClearSanitize}>NEW TARGET</button>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
      ...
      {/* History Route */}
      <Route path="/StegoSphere/history" element={
        <Dashboard showHistory={true} onNavigate={navigateToTab} />
      } />
      ...
      {/* Catch-all redirect */}
      <Route path="*" element={<div className="min-h-screen flex items-center justify-center text-green-500 font-mono">404 - Page Not Found</div>} />
    </Routes>
  );
}